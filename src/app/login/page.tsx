...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Sync session before redirecting
    ...
    <div className="w-full lg:grid lg:min-h-[calc(100vh-4rem)] lg:grid-cols-2">
      ...
      <div className="relative hidden h-full flex-col bg-muted lg:flex overflow-hidden">
        ...
        <div className="relative z-20 flex h-full flex-col p-10 text-white justify-between">
          ...
          >
            <h2 className="text-4xl font-bold leading-tight">Elevate your MUN experience to the next level.</h2>
            <p className="text-lg text-white/80">
              Join thousands of delegates, chairs, and organizers who trust MUN Platform to manage their entire Model UN journey.
            </p>
            
            <div className="flex items-center gap-4 pt-4">
              <div className="flex -space-x-4">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="w-10 h-10 rounded-full border-2 border-indigo-900 bg-white/10 backdrop-blur-md" />
                ))}
              </div>
              <div className="text-sm">
                <span className="font-bold">10k+</span> users worldwide
              </div>
            </div>
          </motion.div>
        </div>
